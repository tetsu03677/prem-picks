...
from google_sheets_client import read_config, list_fixtures, list_bets, upsert_bet
...
for f in sorted(fixtures, key=lambda r: str(r.get("kickoff_jst", ""))):
    ...
    pick = cols[0].radio("予想", ["Home", "Draw", "Away"], index=pick_default, key=pick_key, horizontal=True, disabled=gw_locked)
    stake_val = int(prev.get("stake") or 0)
    stake = cols[1].number_input("掛金", min_value=0, max_value=max_total, step=step, value=stake_val, key=stake_key, disabled=gw_locked)
